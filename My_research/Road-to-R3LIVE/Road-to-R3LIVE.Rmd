...
- 따라서, 내 예측한 state가 맞다고 가정하면
$$\mathbf{z}_t \sim \mathcal{N}(\mathbf{C}_t \mathbf{x}_t, \mathbf{Q})$$
...
- 이를 함수로 모아다가 중한것들만 남기곤 다시 써보면
$$\mathbf{J}(x_t) = ((x_t-\bar{\mu}_{x_t})^\top \bar{\Sigma}_{x_t}^{-1}(x_t-\bar{\mu}_{x_t}) + 
(z_t-\mathbf{C}_t x_t)^\top \mathbf{Q}^{-1}(z_t-\mathbf{C}_t  x_t))$$
...
- 전개해봅시다
.f-80.nl-05[
$$\mathbf{J}(\mathbf{x}_t)=\Bigg( \mathbf{x}_t^\top \bar{\Sigma}_{x_t}^{-1} \mathbf{x}_t - 2\bar{\mu}_{x_t}^\top \bar{\Sigma}_{x_t}^{-1} 
...
\mathbf{x}_t - 2\bar{\mu}_{x_t}^\top \bar{\Sigma}_{x_t}^{-1} \mathbf{x}_t - 2 \mathbf{z}_t^\top \mathbf{Q}^{-1} \mathbf{C}_t \mathbf{x}_t+ const \Bigg)$$
...
---
# Kalman Filter - Kalman Gain derivation
- Kalman Gain을 구하기 위해서, predicted mean으로부터 예상되는 관측값 잠깐 활용
