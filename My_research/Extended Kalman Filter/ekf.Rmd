...
- 선형화 및 근사
  - 기울기를 통한 함수의 선형화 뿐만 아니라, 구하고자 하는 함수값에 최대한 근접할 수 있도록 여러 함수의 조합으로 근사
   - 1차, 2차, n차 항의 조합으로 기존 함수를 근사 할 수 있음
...
---
# Extended Kalman Filter - 2nd order approximation
.pull-left[
...
- EKF에서는, **이전 state의 평균을 기준**으로 선형화된다고 가정
...
- 저게 왜 저것만 남냐면요, 사실은,,
$$\mathbf{x}_{t-1}^{i}=\boldsymbol{\mu}_{t-1}+\delta\mathbf{x}_{t-1}^{i} \text{,} \quad \frac{1}{N}\sum_{i=1}^{N}\delta\mathbf{x}_{t-1}^{i}=\mathbf{0}$$
$$\mathbf{x}_t^{i}\approx f(\boldsymbol{\mu}_{t-1},\mathbf{u}_t)+\mathbf{J}_t\delta\mathbf{x}_{t-1}^{i}+\boldsymbol{\epsilon}_t^{i}$$

$$\mathbf{\bar{\mu}}_t = \frac{1}{N}\sum_{i=1}^{N}\mathbf{x}_t^{i} \approx \frac{1}{N}\sum_{i=1}^{N}\left[f(\boldsymbol{\mu}_{t-1},\mathbf{u}_t)+\mathbf{J}_t\delta\mathbf{x}_{t-1}^{i}
+\boldsymbol{\epsilon}_t^{i}\right] = f(\boldsymbol{\mu}_{t-1},\mathbf{u}_t)+\mathbf{J}_t
\left(\frac{1}{N}\sum_{i=1}^{N}\delta\mathbf{x}_{t-1}^{i}\right)+\frac{1}{N}\sum_{i=1}^{N}\boldsymbol{\epsilon}_t^{i}$$

$$\frac{1}{N}\sum_{i=1}^{N}\delta\mathbf{x}_{t-1}^{i}=
\mathbf{0},\qquad\frac{1}{N}\sum_{i=1}^{N}\boldsymbol{\epsilon}_t^{i}\approx\mathbf{0}$$

$$\therefore\quad\mathbf{\bar{\mu}}_t \approx f(\boldsymbol{\mu}_{t-1},\mathbf{u}_t)$$
---
# Extended Kalman Filter - Prediction step

- Covariance?
  - 앞에 구한 mean을 가져와서 식을 다시 써보면
...
\left[
\left\{
\mathbf{J}_t(\mathbf{x}_{t-1}-\boldsymbol{\mu}_{t-1})
...
(\mathbf{x}_{t-1}-\boldsymbol{\mu}_{t-1})^\top
\right]
\mathbf{J}_t^\top
+
\mathbb{E}
\left[
\boldsymbol{\epsilon}_t\boldsymbol{\epsilon}_t^\top
\right]
\\[4pt]
&=
\boxed{
\mathbf{J}_t\mathbf{\Sigma}_{t-1}\mathbf{J}_t^\top
+\mathbf{R}_t
}
\end{aligned}
$$
...
---
# Extended Kalman Filter 
- Kalman Gain과 업데이트도 기존 KF와 크게 다르지 않음
